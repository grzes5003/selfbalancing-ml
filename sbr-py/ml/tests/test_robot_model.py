...
class TestRobotInterface(TestCase):
    ...
    def test_load_qlearn_real(self):
        file_name = 'tmp/16231037_result.npz'
        env = RobotModel()
        ml_model = QModel((NUM_OF_STATES, 6), 5)
        with np.load(file_name) as data:
            ml_model.set_q_table(data['q_table'])
            scores = data['scores'].tolist()
        n_episodes = 250
        for e in range(len(scores), n_episodes):
            print("--------------------------")
            env.done = False
            _, reward, discount, current_state = env.reset()
            current_state = tuple(current_state.reshape(-1))
        ...
        print(ml_model.q_table)
        print(f"Scores {scores} in {n_episodes} episodes")
        np.savez(f'tmp/{datetime.now().strftime("%d%H%M%S")}_result', q_table=ml_model.q_table,
                 scores=np.array(scores, np.int32))
        env.close_()
    ...
    def test_reward(self):
        ...
        env.done = False
        while True:
            obs, reward = env.fake_reward()
            print(f'{obs[0]}: {reward:>10}')
